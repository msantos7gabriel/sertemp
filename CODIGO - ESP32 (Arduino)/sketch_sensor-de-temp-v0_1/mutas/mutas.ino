...
void loop() {
  if (Firebase.ready() && (millis() - sendDataPrevMillis > 60000 || sendDataPrevMillis == 0)) {
    ...
    String path = "mutas/historico/" + String(dateOnly) + "/" + String(timeOnly);
    ...
    Firebase.RTDB.setFloat(&fbdo, "mutas/Temperature", temp);
    Firebase.RTDB.setFloat(&fbdo, "mutas/Humidity", humidity);
    Firebase.RTDB.setString(&fbdo, "mutas/lastUpdate", dateTime);
    Firebase.RTDB.setString(&fbdo, "mutas/Altitude", altitude);
    Firebase.RTDB.setString(&fbdo, "mutas/SealevelPressure", pressure);

...
void loop() {
  if (Firebase.ready() && (millis() - sendDataPrevMillis > 60000 || sendDataPrevMillis == 0)) {
    ...
    String path = "alvorada/historico/" + String(dateOnly) + "/" + String(timeOnly);
    ...
    Firebase.RTDB.setFloat(&fbdo, "alvorada/Temperature", temp);
    Firebase.RTDB.setFloat(&fbdo, "alvorada/Humidity", humidity);
    Firebase.RTDB.setString(&fbdo, "alvorada/lastUpdate", dateTime);
    Firebase.RTDB.setString(&fbdo, "alvorada/Altitude", altitude);
    Firebase.RTDB.setString(&fbdo, "alvorada/SealevelPressure", pressure);
